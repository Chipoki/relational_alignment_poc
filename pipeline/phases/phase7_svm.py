"""Phase 7 – SVM Decoding (Mei et al. 2022).
...
Produces:
  ...
  • stats/phase7_svm.json with all numbers
"""
...
def run(
    ...
    logger.info("=" * 60)
    logger.info("PHASE 7 – SVM Decoding  (Mei et al. 2022)")
    logger.info("=" * 60)
    ...
    for subj in subjects:
        ...
        for state in ("conscious", "unconscious"):
            plotter.plot_decoding_by_roi(
                ...
                roi_names=roi_names,
                subject_id=sid,
                save_name=f"phase7_svm_{sid}_{state}.png",
            )
    ...
    save_json(json_out, Path(settings.stats_dir) / "phase7_svm.json")
